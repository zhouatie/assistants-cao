...
exports.runConfigCli = runConfigCli;
...
function listModels() {
    ...
    console.log(`${'模型名称'.padEnd(15)} ${'默认'.padEnd(8)} ${'API基础URL'.padEnd(25)} ${'模型名'}`);
    console.log('-'.repeat(60));
    for (const [name, modelConfig] of Object.entries(models)) {
        const isDefault = name === defaultModel ? '✓' : '';
        console.log(`${name.padEnd(15)} ${isDefault.padEnd(8)} ${modelConfig.api_base.padEnd(25)} ${modelConfig.model}`);
    }
    console.log('-'.repeat(60));
}
/**
        ...
        switch (choice.trim()) {
            case '1':
                ...
                if (name && apiBase && model) {
                    ...
                    const result = config.addModel(name.trim(), apiBase.trim(), model.trim(), apiKey.trim() || undefined);
                    if (result) {
 ...
 * @param args 命令行参数
 */
function runConfigCli(args) {
    // 简易参数解析
    if (args.length === 0 || args[0] === 'list') {
        listModels();
    }
    else if (args[0] === 'add' && args.length >= 4) {
        const [_, name, apiBase, model, apiKey] = args;
        ...
        if (args.length >= 2) {
            try {
                ...
                console.error(`导出配置失败: ${e.message}`);
            }
        }
        else {
